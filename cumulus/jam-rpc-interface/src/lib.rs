//! Implementation of the `jam-interface` traits over one websocket to a JAM node (JIP-2).
//!
...
//! maps the `Node` surface onto the three `jam-interface` traits.
...
const LOG_TARGET: &str = "jam-rpc-interface";
...
impl JamChainSource for JamRpcInterface {
	...
	async fn finalized_block_stream(&self) -> Result<BoxStream<'static, BlockDesc>> {
		self.register_block_listener(WorkerMessage::RegisterFinalizedBlockListener).await
	}
...
impl JamStateSource for JamRpcInterface {
	...
	async fn service_value_stream(
		...
		Ok(resubscribing_value_stream(
			...
			move |client| {
				let key = key.clone();
				Box::pin(async move { client.subscribe_service_value(service, &key, finalized).await })
			},
		))
	}
}
