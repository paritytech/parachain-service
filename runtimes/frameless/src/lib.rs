...
/// One scripted side effect, driven from the block body in tests via
/// [`Config::Mock`]. Most variants name an `UpwardMessage` the guest emits
/// through `send_upward_message`; the rest drive the entry point's own head
/// declarations to exercise §4.2 ABI violations.
...
mod host {
	...
	pub fn run_action(action: &MockAction) {
		...
		// §4.3: every side effect now travels as one SCALE-encoded `UpwardMessage`
		// through a single host call, so this only has to build the message.
		let msg = match action {
			...
			MockAction::EjectService { service } => {
				UpwardMessage::EjectService { service: *service }
			},
			...
			MockAction::CreateService(args) => UpwardMessage::CreateService(args.clone()),
			MockAction::Forget { target, hash, len } => {
				UpwardMessage::Forget { target: *target, hash: *hash, len: (*len).into() }
			},
			...
			MockAction::SetValidatorKeys { keys, is_last } => UpwardMessage::SetValidatorKeys {
				keys: keys
					.chunks_exact(336)
			...
			MockAction::CleanUpBucketsUpTo { bucket_id } => {
				UpwardMessage::CleanUpBucketsUpTo(*bucket_id)
			},
			...
			MockAction::ReportError(data) => {
				unsafe { report_error_raw(data.as_ptr() as u32, data.len() as u32) }
				return;
			},
			MockAction::DuplicateSetHead(_) | MockAction::SkipHeadDeclarations => return,
		};
